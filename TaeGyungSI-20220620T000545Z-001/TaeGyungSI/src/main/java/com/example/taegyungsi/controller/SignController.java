...
//    null => 스프링 객체
//    SignController에 싱글톤 = 1;
//    Sign2Controller에 싱글톤 + 1;
    @Autowired
...
//            2) 웹토큰 전송 + 유저정보 전송 : Vue로 전송
            return ResponseEntity.ok(new JwtResponse(jwt, result.getId(),
                    result.getUsername(),
                    result.getEmail(),
                    roles));
    }
//    회원가입 메뉴(sign up)
    ...
    public ResponseEntity<?> addUser(HttpServletRequest request, @RequestBody User signupUser){

//        임시 유저 객체 정의
        User user = signupUser;
//        유저에 role(역할) : ROLE_USER, 이 사이트는 롤이 1개임
        user.setRoles("ROLE_USER");
//        signupUser.getPassword(): 암호화 되기전 패스워드
//        passwordEncoder.encode : 패스워드 암호화 됨
